    ...
    <div className="w-full max-w-[1440px] mx-auto px-2 sm:px-4 md:px-[38px] overflow-x-auto">
      <div className="flex items-center justify-center gap-3 md:gap-7 min-w-0">
        ...
        <div className="flex items-center gap-4 sm:gap-6 md:gap-8 overflow-x-auto scrollbar-hide justify-start md:justify-center pb-2 md:pb-0">
          {data.map((category) => (
            <div key={category.id} className="flex flex-col items-center gap-2 md:gap-3 flex-shrink-0">
              <div className="relative w-[100px] h-[100px] sm:w-[120px] sm:h-[120px] md:w-[148px] md:h-[148px] rounded-xl md:rounded-[17.41px] border border-[#901CDB]/20 overflow-hidden">
                <img 
                  src={category.image} 
                  alt={category.name}
              ...
              <span className="text-xs sm:text-sm md:text-base font-medium text-[#353945] text-center">{category.name}</span>
            </div>
          ))}
        </div>
